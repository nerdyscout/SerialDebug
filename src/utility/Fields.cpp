...
Fields::Fields(String str, const char delimiter, bool ignoreEmpty)
{
	...
	do
	{
		...
		// DEBUG_SERIAL.printf("pos=%u last=%u\r\n", pos, lastPos);
		...
		if (pos >= 0)
		{
			...
			if (!(ignoreEmpty && pos == lastPos))
			{
				...
				//             	DEBUG_SERIAL.printf("field %s len=%u pos=%u last=%u\r\n", field.c_str(), field.length(), pos, lastPos);
				// #else
				//             	Serial.print ("field ");
				//             	Serial.print (field);
				//				Serial.print (" len=");
				...
				if (!(ignoreEmpty && field.length() == 0))
				{
					...
					//            		DEBUG_SERIAL.print("push_back ");
					//            		DEBUG_SERIAL.println(field);
				}
			}
	...
	//
	// #if defined ESP8266 || defined ESP32
	//    DEBUG_SERIAL.printf("fields.size() = %u \r\n", fields.size());
	//    for (uint8_t i=1; i <= fields.size(); i++) {
	//		DEBUG_SERIAL.printf("field %u = %s \r\n", i,  getString(i).c_str());
	//    }
	// #else
	//    DEBUG_SERIAL.print("fields.size() = ");
	//    DEBUG_SERIAL.println(fields.size());
	//    for (uint8_t i=1; i <= fields.size(); i++) {
	//		DEBUG_SERIAL.print("field ");
	//		DEBUG_SERIAL.print(i);
	//		DEBUG_SERIAL.print(" = ");
	//		DEBUG_SERIAL.println(getString(i).c_str());
	//    }
	// #endif
}
