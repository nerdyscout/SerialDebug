...
// Serial interface to print out messages
#ifndef DEBUG_SERIAL
...
#define _debug(level, fmt, ...)                                     \
	{                                                               \
		...
		{                                                           \
			DEBUG_SERIAL.printf("(%c p:^%04lu %s C%d) " fmt "\r\n", \
		...
		{                                                           \
			DEBUG_SERIAL.printf("(%c %lu %s C%d) " fmt "\r\n",      \
								level, millis(),                    \
								__func__, xPortGetCoreID(),         \
								##__VA_ARGS__);                     \
		}                                                           \
...
#define _debug(level, fmt, ...)                                     \
	{                                                               \
		...
		if (_debugShowProfiler)                                     \
		{                                                           \
			DEBUG_SERIAL.printf("(%c p:^%04lu C%d) " fmt "\r\n",    \
								level, (millis() - _debugLastTime), \
								xPortGetCoreID(),                   \
								##__VA_ARGS__);                     \
			_debugLastTime = millis();                              \
		...
		{                                                           \
			DEBUG_SERIAL.printf("(%c %lu C%d) " fmt "\r\n",         \
								level, millis(),                    \
								xPortGetCoreID(),                   \
								##__VA_ARGS__);                     \
		}                                                           \
...
#define _debug(level, fmt, ...)                                     \
	{                                                               \
		...
		if (_debugShowProfiler)                                     \
		{                                                           \
			DEBUG_SERIAL.printf("(%c p:^%04lu %s) " fmt "\r\n",     \
								level, (millis() - _debugLastTime), \
								__func__,                           \
								##__VA_ARGS__);                     \
			_debugLastTime = millis();                              \
		...
		{                                                           \
			DEBUG_SERIAL.printf("(%c %lu %s) " fmt "\r\n",          \
								level, millis(),                    \
								__func__,                           \
								##__VA_ARGS__);                     \
		}                                                           \
...
#define _debug(level, fmt, ...)                                     \
	{                                                               \
		...
		if (_debugShowProfiler)                                     \
		{                                                           \
			DEBUG_SERIAL.printf("(%c p:^%04lu) " fmt "\r\n",        \
								level, (millis() - _debugLastTime), \
								##__VA_ARGS__);                     \
			_debugLastTime = millis();                              \
		}                                                           \
...
#define debugA(fmt, ...) \
	if (!_debugSilence)  \
	_debug('A', fmt, ##__VA_ARGS__)

// With level

#define debugV(fmt, ...)                                      \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_VERBOSE) \
	_debug('V', fmt, ##__VA_ARGS__)
#define debugD(fmt, ...)                                    \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_DEBUG) \
	_debug('D', fmt, ##__VA_ARGS__)
#define debugI(fmt, ...)                                   \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_INFO) \
	_debug('I', fmt, ##__VA_ARGS__)
#define debugW(fmt, ...)                                   \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_WARN) \
	_debug('W', fmt, ##__VA_ARGS__)

// For errors (always showed)

#define debugE(fmt, ...) \
	if (!_debugSilence)  \
	_debug('E', fmt, ##__VA_ARGS__)
...
#define debugIsrA(fmt, ...) \
	if (!_debugSilence)     \
	_debugIsr('A', fmt, ##__VA_ARGS__)

// With level

#define debugIsrV(fmt, ...)                                   \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_VERBOSE) \
	_debugIsr('V', fmt, ##__VA_ARGS__)
#define debugIsrD(fmt, ...)                                 \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_DEBUG) \
	_debugIsr('D', fmt, ##__VA_ARGS__)
#define debugIsrI(fmt, ...)                                \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_INFO) \
	_debugIsr('I', fmt, ##__VA_ARGS__)
#define debugIsrW(fmt, ...)                                \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_WARN) \
	_debugIsr('W', fmt, ##__VA_ARGS__)

// For errors (always showed)

#define debugIsrE(fmt, ...) \
	if (!_debugSilence)     \
	_debugIsr('E', fmt, ##__VA_ARGS__)
...
#define printLevel(level, x, ...)             \
	{                                         \
		...
		DEBUG_SERIAL.print(x, ##__VA_ARGS__); \
	}
...
#define printlnLevel(level, x, ...)             \
	{                                           \
		...
		DEBUG_SERIAL.println(x, ##__VA_ARGS__); \
...
#define printA(x, ...)  \
	if (!_debugSilence) \
	printLevel('A', x, ##__VA_ARGS__)
#define printlnA(x, ...) \
	if (!_debugSilence)  \
	printlnLevel('A', x, ##__VA_ARGS__)

// With level

#define printV(x, ...)                                        \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_VERBOSE) \
	printLevel('V', x, ##__VA_ARGS__)
#define printD(x, ...)                                      \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_DEBUG) \
	printLevel('D', x, ##__VA_ARGS__)
#define printI(x, ...)                                     \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_INFO) \
	printLevel('I', x, ##__VA_ARGS__)
#define printW(x, ...)                                     \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_WARN) \
	printLevel('W', x, ##__VA_ARGS__)

#define printlnV(x, ...)                                      \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_VERBOSE) \
	printlnLevel('V', x, ##__VA_ARGS__)
#define printlnD(x, ...)                                    \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_DEBUG) \
	printlnLevel('D', x, ##__VA_ARGS__)
#define printlnI(x, ...)                                   \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_INFO) \
	printlnLevel('I', x, ##__VA_ARGS__)
#define printlnW(x, ...)                                   \
	if (!_debugSilence && _debugLevel >= DEBUG_LEVEL_WARN) \
	printlnLevel('W', x, ##__VA_ARGS__)

// For errors (always showed)

#define printE(x, ...)  \
	if (!_debugSilence) \
	printLevel('E', x, ##__VA_ARGS__)
#define printlnE(x, ...) \
	if (!_debugSilence)  \
	printlnLevel('E', x, ##__VA_ARGS__)
